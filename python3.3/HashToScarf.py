...
#                error(self.error_level, "Required Element: Could not find any Locations in BugInstance: %s" % self.bugID)
        ...
        self.output.write(etree.tostring(bug, pretty_print = True, encoding="unicode"))
        bug.clear()
...
###########Writer a metric##################################################
    def addMetric(self, metricHash):
        ...
        self.output.write(etree.tostring(metric, pretty_print = True, encoding="unicode"))
...
############Add summary from written elements##############################################################
    def addSummary(self):
        ...
        if self.bugSummaries:
            ...
            self.output.write(etree.tostring(summaries, pretty_print = True, encoding="unicode"))
